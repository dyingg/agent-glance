...
import { createGlanceBuffer } from "../src/glance-buffer.js";

describe("GlanceBuffer", () => {
  describe("read", () => {
    test("empty buffer returns empty html and version 0", () => {
      const buf = createGlanceBuffer();
    ...
    test("initialHtml surfaces with version 0 until first mutation", () => {
      const buf = createGlanceBuffer({ initialHtml: "<p>hi</p>" });
  ...
  describe("write", () => {
    ...
    test("fires onChange with new html", async () => {
      ...
      const buf = createGlanceBuffer({ onChange });
      buf.write("<p>x</p>");
  ...
  describe("hide", () => {
    test("sets html to empty string and bumps version", () => {
      const buf = createGlanceBuffer({ initialHtml: "<p>x</p>" });
    ...
    test("fires onChange with empty string", async () => {
      const onChange = vi.fn();
      const buf = createGlanceBuffer({ initialHtml: "<p>x</p>", onChange });
      buf.hide();
      await Promise.resolve();
  ...
  describe("edit — gate", () => {
    test("MustReadFirst when client has never read", () => {
      ...
      expect(result).toEqual({
        ok: false,
        code: "MustReadFirst",
        message: expect.stringContaining("read_glance"),
      });
    });
    ...
    test("Ambiguous when old_string matches >1 without replace_all", () => {
      const buf = createGlanceBuffer({ initialHtml: "<b>x</b><b>x</b>" });
      buf.read("c");
  ...
  describe("edit — happy path", () => {
    ...
    test("fires onChange with the new html", async () => {
      const onChange = vi.fn();
      const buf = createGlanceBuffer({ initialHtml: "<p>hi</p>", onChange });
      buf.read("c");
